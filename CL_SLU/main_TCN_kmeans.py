...
from utils.utils import trunc, get_kdloss,get_finetuning_dataset, SpecAugment,collate_batch, KMeans
import time
import datetime
import json
...
def get_args_parser():
    ...
    parser.add_argument('--data_path', type=str, default='/Users/umbertocappellazzo/Desktop/PHD',help='path to dataset')
...
def main(args):
    ...
    #wandb.init(project="CL FSC (DyTox)", name="TCN_rehe_KMEANS_30centroids",entity="umbertocappellazzo",config = {"lr": args.lr, "weight_decay":args.weight_decay, "epochs":args.epochs, "batch size": args.batch_size})
   ...
   # KD DISTILLATION. 
    ...
    ###########################################################################
        ...
        if task_id == 1:
            memory_counters = [0]
            for i in range(50):
                memory_counters.append((i+1)*2)
            for i in range(17):
                memory_counters.append(100+i)
            memory_counters.append(118)
            memory_counters.append(120)
                
        elif task_id >1:
            memory_counters = [0]
            print(len(train_loader))
            print(data_mem.shape[0])
            for i in range(len(train_loader)):
                memory_counters.append((data_mem.shape[0]//len(train_loader))*(i+1))
            if  data_mem.shape[0] % (len(train_loader)) != 0:
                memory_counters[-1] += data_mem.shape[0] % (len(train_loader))
        ...
        for epoch in range(epochs):
            ...
            for (count,(x,y,t)) in enumerate(train_loader):
                ...
                if task_id >0:
                    x = torch.cat((x,data_mem[memory_counters[count]:memory_counters[count+1],:]))
                    y = torch.cat((y,y_mem[memory_counters[count]:memory_counters[count+1],:]))
                    t = torch.cat((t,t_mem[memory_counters[count]:memory_counters[count+1]]))
            ...
            if args.eval_every and (epoch+1) % args.eval_every == 0:
                ...
                with torch.no_grad():
                    for x_valid, y_valid, t_valid in test_loader:
                        ...
                        predic_valid = model(x_valid)#['logits']
                        
                        
                        #test_loss += criterion(predic_valid,y_valid[:,0].cuda()).item()
                        test_loss += criterion(predic_valid,y_valid[:,0]).item()
        ...
        if memory is not None:
            ...
            current_classes = scenario_train[task_id].get_classes()
            if task_id >0:
                current_classes = current_classes[-3:]
            y_t = exp_train._y#; t_t = exp_train._t
            
            memory_per_class = 30
            data_2add = []; y_2add = []; t_2add = []
            for curr_class in current_classes:
                print(f"Applying k-means to class {curr_class}: ")
                indexes = set(np.where(np.isin(y_t[:,0], [curr_class]))[0])
                indexes = np.array(list(indexes))
                class_data,class_y,class_t = exp_train.get_samples(indexes=indexes)
                #plot_spectrogram(class_data[0,:,:])
                print(class_data.shape)
                
                #data_2add.append(class_data[0:memory_per_class,:]); y_2add.append(class_y[:memory_per_class]); t_2add.append(class_t[:memory_per_class])
                
                _, centroids = KMeans(class_data,memory_per_class,Niter=1)
                plot_spectrogram(centroids[0,:,:])
                
                
                
                
                data_2add.append(centroids); y_2add.append(class_y[:centroids.shape[0]]); t_2add.append(class_t[:centroids.shape[0]])
            if task_id ==0:
                data_mem = torch.cat(data_2add); y_mem = torch.cat(y_2add); t_mem = torch.cat(t_2add)
            else:
                data_mem = torch.cat((data_mem,torch.cat(data_2add))); y_mem = torch.cat((y_mem,torch.cat(y_2add))); t_mem = torch.cat((t_mem,torch.cat(t_2add)))
            
            print(data_mem.shape); print(y_mem.shape); print(t_mem.shape)
            
            #memory.add(data_mem,y_mem,t_mem,z=None)
            ...
            #memory.add(*scenario_train[task_id].get_samples(range(len(scenario_train[task_id]))),z=None)
